...
const fifaAutofillHandler = (profileInfo, tab, card) => {
  ...
  // Check for OTP input - multiple selectors for FIFA verification page
  const otp = document.querySelector('#otp') ||
              document.querySelector('input[placeholder="Enter Code"]') ||
              document.querySelector('input[placeholder*="code" i]') ||
              document.querySelector('input[name="otp"]');

  console.log('[CSV] OTP input found:', !!otp);
  console.log('[CSV] Profile email:', profileInfo['acc_email']);

  if (otp || window.location.href.includes('authorize') || window.location.href.includes('verify')) {
    console.log('[CSV] Triggering OTP fetch for:', profileInfo['acc_email']);
    chrome.runtime.sendMessage({
      action: 'getEmailOTP',
...
document.addEventListener(
  ...
  function (e) {
    ...
    } else if (e.altKey && e.code && e.code == 'KeyQ') {
      // Alt+Q - Show CSV Profile Selector
      e.preventDefault();
      showCSVProfileSelector();
    }
  },
  true
);

// ========== CSV PROFILE SELECTOR (Alt+Q) ==========
let csvSelectorOpen = false;
let csvSelectorElement = null;

async function showCSVProfileSelector() {
  // Toggle off if already open
  if (csvSelectorOpen && csvSelectorElement) {
    csvSelectorElement.remove();
    csvSelectorElement = null;
    csvSelectorOpen = false;
    return;
  }

  // Request accounts from background
  chrome.runtime.sendMessage({ action: 'getCSVAccounts' }, function(response) {
    if (!response || !response.accounts || response.accounts.length === 0) {
      alert('No accounts found in accounts.csv');
      return;
    }

    const accounts = response.accounts;
    const selectedRow = response.selectedRow || 0;

    // Create selector UI
    csvSelectorElement = document.createElement('div');
    csvSelectorElement.id = 'csv-profile-selector';
    csvSelectorElement.style.cssText = `
      position: fixed;
      top: 50%;
      left: 50%;
    ...
    let html = '<h3 style="margin-top:0;color:#4da6ff;border-bottom:1px solid #333;padding-bottom:10px;">📋 Select Profile (Alt+Q to close)</h3>';
    html += '<div style="max-height:400px;overflow-y:auto;">';

    accounts.forEach((acc, index) => {
      const isSelected = index === selectedRow;
      const bgColor = isSelected ? '#2d5a27' : '#16213e';
      const border = isSelected ? '2px solid #4da6ff' : '1px solid #333';
      html += `
        <div class="csv-profile-item" data-index="${index}" style="
          background: ${bgColor};
          border: ${border};
          border-radius: 8px;
          padding: 12px;
          ...
          transition: all 0.2s;
        ">
          <div style="font-weight:bold;color:#4da6ff;">${index + 1}. ${acc.acc_email || acc.email || 'No email'}</div>
          <div style="font-size:12px;color:#888;margin-top:4px;">
            ${acc.fname || ''} ${acc.lname || ''} ${acc.address_city || acc.city || ''}
          </div>
          ${isSelected ? '<span style="color:#4da6ff;font-size:11px;">✓ SELECTED</span>' : ''}
        </div>
      `;
    });

    html += '</div>';
    html += '<div style="margin-top:15px;text-align:center;color:#666;font-size:12px;">Click a profile to select it</div>';

    csvSelectorElement.innerHTML = html;
    document.body.appendChild(csvSelectorElement);
    csvSelectorOpen = true;

    // Add click handlers
    csvSelectorElement.querySelectorAll('.csv-profile-item').forEach(item => {
      item.addEventListener('mouseenter', () => {
        if (!item.style.background.includes('2d5a27')) {
          item.style.background = '#1f3460';
        }
      });
      item.addEventListener('mouseleave', () => {
        const idx = parseInt(item.dataset.index);
        if (idx !== selectedRow) {
          item.style.background = '#16213e';
        }
      });
      item.addEventListener('click', () => {
        const newIndex = parseInt(item.dataset.index);
        chrome.runtime.sendMessage({ action: 'setCSVRow', row: newIndex }, function(resp) {
          // Close selector and show confirmation
          csvSelectorElement.remove();
          csvSelectorElement = null;
          csvSelectorOpen = false;
          ...
          notif.textContent = `Profile ${newIndex + 1} selected: ${accounts[newIndex].acc_email || accounts[newIndex].email}`;
    ...
    setTimeout(() => {
      document.addEventListener('click', function closeOnOutside(e) {
        if (csvSelectorElement && !csvSelectorElement.contains(e.target)) {
          csvSelectorElement.remove();
          csvSelectorElement = null;
          csvSelectorOpen = false;
