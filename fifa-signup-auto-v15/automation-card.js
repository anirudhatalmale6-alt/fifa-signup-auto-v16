...
(function() {
    ...
    // ========== MAKE TICKET SUMMARY BIGGER ==========
    function enlargeTicketSummary() {
        // Target the specific elements - "40 tickets" and price are separate <p> elements
        const allP = document.querySelectorAll('p');
        for (const el of allP) {
            const text = (el.textContent || '').trim();

            // Match "XX tickets" pattern
            if (text.match(/^\d+\s*tickets?$/i)) {
                if (!el.classList.contains('fifa-enlarged')) {
                    el.classList.add('fifa-enlarged');
                    el.style.cssText = 'font-size: 64px !important; font-weight: bold !important; color: #00ff00 !important; text-shadow: 2px 2px 4px #000 !important; max-width: none !important; overflow: visible !important;';
                    console.log('[FIFA] Enlarged tickets:', text);
                }
            }

            // Match price with USD pattern (e.g., "29,580.00 USD" or "• 29,580.00 USD")
            if (text.match(/[\d,\.]+\s*USD/i) && text.length < 30) {
                if (!el.classList.contains('fifa-enlarged')) {
                    el.classList.add('fifa-enlarged');
                    el.style.cssText = 'font-size: 64px !important; font-weight: bold !important; color: #00ff00 !important; text-shadow: 2px 2px 4px #000 !important; max-width: none !important; overflow: visible !important; white-space: nowrap !important;';
                    console.log('[FIFA] Enlarged price:', text);
                }
            }
        }

        // Also check for updatedCurrency class specifically
        const currencyElements = document.querySelectorAll('.updatedCurrency, [class*="updatedCurrency"]');
        for (const el of currencyElements) {
            if (!el.classList.contains('fifa-enlarged')) {
                el.classList.add('fifa-enlarged');
                el.style.cssText = 'font-size: 64px !important; font-weight: bold !important; color: #00ff00 !important; text-shadow: 2px 2px 4px #000 !important; max-width: none !important; overflow: visible !important; white-space: nowrap !important;';
                console.log('[FIFA] Enlarged currency element');
            }
        }
    }

    // Watch for ticket summary appearing
    const observer = new MutationObserver(function(mutations) {
        enlargeTicketSummary();
    });
    observer.observe(document.body, { childList: true, subtree: true });

    // Check periodically too
    setInterval(enlargeTicketSummary, 500);
