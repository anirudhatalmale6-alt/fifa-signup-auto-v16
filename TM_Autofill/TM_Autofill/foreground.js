...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  // Handle Alt+X autofill trigger
  if (message.action === 'autofill') {
    console.log('[TM] Alt+X autofill triggered');
    // Get profile data from storage and run autofill
    chrome.storage.local.get(['accounts', 'selectedRow'], function(result) {
      const accounts = result.accounts || [];
      const selectedRow = result.selectedRow || 0;
      if (accounts.length > 0 && accounts[selectedRow]) {
        const account = accounts[selectedRow];
        // Map CSV fields to profileInfo format
        const profileInfo = {
          acc_email: account.email || '',
          tm_pass: account.password || account.tm_pass || '',
          fname: account.first_name || '',
          lname: account.last_name || '',
          full_name: account.full_name || `${account.first_name || ''} ${account.last_name || ''}`.trim(),
          tel: account.phone || '',
          address_zip: account.zip_code || '',
          address_city: account.city || '',
          address_state: account.province || '',
          address_street: account.address || '',
          card_number: account.card_number || '',
          card_expiry: account.card_expiry || '',
          card_cvc: account.cvc || '',
          country: account.country || 'USA',
          gender: account.gender || 'Prefer Not to Say',
          language: account.language || 'English'
        };
        console.log('[TM] Running autofill with profile:', profileInfo.acc_email);
        // Trigger autofill based on current page
        triggerAutofill(profileInfo);
      }
    });
  }
  return true;
...
// Function to trigger autofill based on current page
function triggerAutofill(profileInfo) {
  const url = window.location.href;
  console.log('[TM] Autofill triggered on:', url);

  // FIFA forms
  if (url.includes('fifa.com') || url.includes('tickets.fifa.com')) {
    console.log('[TM] FIFA autofill running');

    // Find all inputs and fill by label text
    const allInputs = document.querySelectorAll('input');
    allInputs.forEach(input => {
      // Find the label - check parent divs for label text
      let labelText = '';
      ...
      for (let i = 0; i < 5 && parent; i++) {
        ...
        // Check for text content that looks like a label
        const textNodes = Array.from(parent.childNodes).filter(n => n.nodeType === 3);
        if (textNodes.length) {
          labelText = textNodes.map(n => n.textContent).join(' ').toLowerCase();
      ...
      // First Name
      if (labelText.includes('first name') || name.includes('firstname') || id.includes('firstname')) {
        changeValueReact(input, profileInfo.fname);
        console.log('[TM] Filled first name:', profileInfo.fname);
      }
      // Last Name
      if (labelText.includes('last name') || name.includes('lastname') || id.includes('lastname')) {
        changeValueReact(input, profileInfo.lname);
        console.log('[TM] Filled last name:', profileInfo.lname);
      }
      // Email
      if (labelText.includes('email') || name.includes('email') || input.type === 'email') {
        changeValueReact(input, profileInfo.acc_email);
        console.log('[TM] Filled email:', profileInfo.acc_email);
      }
      // Password
      if (input.type === 'password' || labelText.includes('password')) {
        changeValueReact(input, profileInfo.tm_pass);
        console.log('[TM] Filled password');
      }
      // Phone
      if (labelText.includes('phone') || name.includes('phone') || input.type === 'tel') {
        changeValueReact(input, profileInfo.tel);
        console.log('[TM] Filled phone:', profileInfo.tel);
      }
    });

    // Handle dropdowns by clicking and selecting
    setTimeout(() => {
      // Country dropdown
      const countryDropdown = findDropdownByLabel('country');
      if (countryDropdown) {
        clickAndSelectDropdown(countryDropdown, profileInfo.country || 'United States');
      }
      ...
      setTimeout(() => {
        ...
        if (genderDropdown) {
          clickAndSelectDropdown(genderDropdown, profileInfo.gender || 'Male');
        }
      }, 300);

      // Language dropdown
      setTimeout(() => {
        const langDropdown = findDropdownByLabel('language');
        if (langDropdown) {
          clickAndSelectDropdown(langDropdown, profileInfo.language || 'English');
        }
      }, 600);
    }, 100);
  }
  ...
  // Ticketmaster forms
  if (url.includes('ticketmaster.com') || url.includes('livenation.com')) {
    console.log('[TM] Ticketmaster autofill');
    ...
    changeValueReact(document.querySelector('input[name="firstName"]'), profileInfo.fname);
    changeValueReact(document.querySelector('input[name="lastName"]'), profileInfo.lname);
    changeValueReact(document.querySelector('input[name="postalCode"]'), profileInfo.address_zip);
    changeValueReact(document.querySelector('input[name="phoneNumber"]'), profileInfo.tel);
  }

  // Generic form fill for any page
  const emailInputs = document.querySelectorAll('input[type="email"], input[name*="email"]');
  const passInputs = document.querySelectorAll('input[type="password"]');
  emailInputs.forEach(el => changeValueReact(el, profileInfo.acc_email));
  passInputs.forEach(el => changeValueReact(el, profileInfo.tm_pass));

  console.log('[TM] Autofill complete');
}

// Find dropdown element by its label text
function findDropdownByLabel(labelText) {
  const allElements = document.querySelectorAll('div, span');
  for (const el of allElements) {
    if (el.textContent.toLowerCase().includes(labelText.toLowerCase())) {
      // Look for dropdown sibling or child
      const parent = el.closest('div');
      if (parent) {
        const dropdown = parent.querySelector('[class*="dropdown"], [class*="select"], select');
        if (dropdown) return dropdown;
        // Check next sibling
        const nextDiv = parent.nextElementSibling;
        if (nextDiv) {
          const dd = nextDiv.querySelector('[class*="dropdown"], [class*="select"], select');
          if (dd) return dd;
        }
      }
    }
...
function clickAndSelectDropdown(dropdown, value) {
  if (!dropdown) return;
  ...
  if (dropdown.tagName === 'SELECT') {
    ...
    if (option) {
      ...
      console.log('[TM] Selected native dropdown:', value);
    }
    return;
  }
  ...
  dropdown.click();
  console.log('[TM] Clicked dropdown, looking for:', value);

  setTimeout(() => {
    // Look for options in the entire document (dropdown might be portaled)
    const allOptions = document.querySelectorAll('[class*="option"], [class*="item"], [role="option"], li');
    for (const opt of allOptions) {
      if (opt.textContent.toLowerCase().includes(value.toLowerCase())) {
        opt.click();
        console.log('[TM] Selected option:', opt.textContent);
        break;
      }
    }
  }, 200);
