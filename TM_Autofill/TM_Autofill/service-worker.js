...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  if (command === 'autofill') {
    // Alt+X - trigger autofill
    debugLog('[TM] Alt+X pressed - triggering autofill');
    if (tab && tab.id) {
  ...
  if (command === 'switch-profile') {
    // Alt+Q - show popup to enter profile number
    debugLog('[TM] Alt+Q pressed - showing profile selector');
      ...
      chrome.storage.local.get(['accounts'], (result) => {
        ...
        chrome.scripting.executeScript({
          ...
          func: (total) => {
            ...
            const existing = document.getElementById('tm-profile-popup');
            if (existing) existing.remove();

            // Create popup container
            const popup = document.createElement('div');
            popup.id = 'tm-profile-popup';
            popup.style.cssText = `
              position: fixed;
              top: 50%;
              left: 50%;
            ...
            popup.innerHTML = `
              <div style="color: #64b5f6; font-size: 16px; margin-bottom: 15px; text-align: center;">
                Select Profile (1-${total})
              </div>
              <input type="number" id="tm-profile-input" min="1" max="${total}"
                style="width: 100%; padding: 12px; font-size: 18px; text-align: center;
                background: #132744; border: 1px solid #1e3a5f; border-radius: 6px;
                color: white; outline: none;"
                placeholder="Enter number" autofocus>
              <div style="color: #90caf9; font-size: 12px; margin-top: 10px; text-align: center;">
            ...
            const input = document.getElementById('tm-profile-input');
            input.focus();
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'setProfile') {
    const profileNum = message.profileNum;
    const selectedRow = profileNum - 1;

    chrome.storage.local.get(['accounts'], (result) => {
      const accounts = result.accounts || [];
      if (selectedRow >= 0 && selectedRow < accounts.length) {
        chrome.storage.local.set({ selectedRow });
        const account = accounts[selectedRow];
        debugLog(`[TM] Switched to profile ${profileNum}: ${account.email || 'Unknown'}`);
          ...
          if (tabs[0] && tabs[0].id) {
            chrome.scripting.executeScript({
              ...
              func: (pNum, pEmail) => {
                ...
                `;
                notify.innerHTML = `✓ Profile ${pNum}: ${pEmail}`;
                document.body.appendChild(notify);
                setTimeout(() => notify.remove(), 2000);
              },
              args: [profileNum, account.email || 'Unknown']
            });
          }
        });
      }
