...
// Default proxy state - starts with DIRECT (no proxy) to pass CAPTCHA
const DEFAULT_STATE = {
  mode: 'direct', // 'direct' or 'proxy' or 'custom'
...
// Service worker wake up - RESTORE saved state instead of resetting
// Only onInstalled/onStartup set initial DIRECT mode (first profile open)
(async () => {
  console.log('[ServiceWorker] Woke up - restoring saved state');
  ...
  try {
    ...
    console.log('[ServiceWorker] Restoring mode:', state.mode);
...
function FindProxyForURL(url, host) {
  return "DIRECT";
    ...
    console.log('[ServiceWorker] ✅ Restored to', state.mode, 'mode');
...
// Initialize extension state - Always reset to DIRECT for CAPTCHA bypass
// This ensures clean state on extension install/update
chrome.runtime.onInstalled.addListener(async () => {
  try {
    console.log('[onInstalled] Extension install/update - resetting to DIRECT mode');

    // Always reset to DIRECT mode
    currentProxyAuth = null;
    const directPACScript = `
function FindProxyForURL(url, host) {
  return "DIRECT";
    ...
    updateBadge('direct');
    console.log('[onInstalled] ✅ Reset to DIRECT mode - ready for CAPTCHA');
...
// Initialize badge on startup - Always reset to DIRECT for CAPTCHA bypass
// This ensures clean state on browser restart
chrome.runtime.onStartup.addListener(async () => {
  try {
    console.log('[onStartup] Browser restart - resetting to DIRECT mode');
...
function FindProxyForURL(url, host) {
  return "DIRECT";
    ...
    updateBadge('direct');
    console.log('[onStartup] ✅ Reset to DIRECT mode - ready for CAPTCHA');
